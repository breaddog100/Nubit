...
function view_status(){
	...
	cd ~/nubit-node/bin
	./nubit das sampling-stats --node.store $store
}
...
function wallet_keys(){
	...
	node_type="light"
	cd ~/nubit-node/bin
	./nkey export $wallet_name --unarmored-hex --unsafe --p2p.network $network --node.type $node_type
}
...
function nub_transfer(){
	...
	store=$HOME/.nubit-light-nubit-alphatestnet-1/
	cd ~/nubit-node/bin
	./nubit state transfer $address $math 400 100000 --node.store $store
}
